...
public class CampaignCreate {
	...
	public static Boolean createCampaign(TableRow row){
	
		try{
				
			String custom_url = URL + "/" + VERSION + "/act_" + String.valueOf(row.getF().get(0).getV()) +"/campaigns";
			...
			parameters.add(new BasicNameValuePair("name", String.valueOf(row.getF().get(1).getV())));
			parameters.add(new BasicNameValuePair("objective", String.valueOf(row.getF().get(2).getV())));
			//parameters.add(new BasicNameValuePair("promoted_object", "{\"product_catalog_id\":\""+ client.getCatalog_id() +"\"}"));
			parameters.add(new BasicNameValuePair("status",String.valueOf(row.getF().get(4).getV())));
